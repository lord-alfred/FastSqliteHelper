...
```
- В SQLite очень просто поймать состояние "_database is locked_", оно возникает в том случае, если **первый поток читает данные из таблицы, а второй поток туда пишет**. Чтобы предотвратить такого рода поведение - нужно заранее продумывать логику работы шаблона и обязательно использовать [оператор блокировки](https://docs.microsoft.com/ru-ru/dotnet/csharp/language-reference/keywords/lock-statement):
```csharp
